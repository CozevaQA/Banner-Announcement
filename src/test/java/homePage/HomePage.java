...
//					cozevaPage.ajaxPreloaderWait(driver);
	...
	public void Test14_check_correctness_from_other_roles() throws Exception {
		Set<String> allRoles = roleAbbrToRoleDetails.keySet();
		List<String> roleNames = new ArrayList<String>();
		List<String> nonSelectedRoleNames = new ArrayList<String>();
		...
		for(String role : allRoles) {
			if(!selectedRoleAbbr.contains(role)) {
//				nonSelectedRoleNames.add(role);
				
				ArrayList<String> list = (ArrayList<String>) roleAbbrToRoleDetails.get(role);
				String name = list.get(1);
				nonSelectedRoleNames.add(name);
			}
		}

		roleNames = roleNames.stream().map(e -> e.trim()).collect(Collectors.toList());
		nonSelectedRoleNames = nonSelectedRoleNames.stream().map(e -> e.trim()).collect(Collectors.toList());
...
//						System.out.println(bannerStrings);
		...
		//Verifying for non selected roles
		Thread.sleep(500);
		for (String custName : customerNames) {
			...
			cozevaPage.ajaxPreloaderWait(driver);
			Thread.sleep(200);
			for (String roleName : nonSelectedRoleNames) {
				...
				// Banner/Announcement verification code
				String expectedResult = String.format("The desired banner or announcement should not be visible for the customer '%s' and role '%s'.", custName,roleName);
				try {
					if (category.startsWith("a") || category.startsWith("A")) {
						Assert.assertTrue(cozevaPage.announcementContainer().isDisplayed());
						List<String> announcementStrings = cozevaPage.announcements().stream().map(e -> e.getText())
